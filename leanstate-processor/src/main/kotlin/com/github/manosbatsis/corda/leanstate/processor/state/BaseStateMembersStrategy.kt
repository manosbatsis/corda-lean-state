...
import com.github.manosbatsis.corda.leanstate.processor.state.mapping.PersistentPropertyMapperCache
...
abstract class BaseStateMembersStrategy(
    ...
    fun getPersistentMappingModes(variableElement: VariableElement): List<PropertyMappingMode> {
        val propModes = variableElement.getAnnotation(LeanStateProperty::class.java)
                ?.mappingModes?.toList()
        return if(propModes != null && propModes.isNotEmpty()) propModes
        else annotatedElementInfo.primaryTargetTypeElement
                        .getAnnotation(LeanStateModel::class.java)!!.mappingModes.toList()
    ...
    ): Pair<String, Boolean>? = variableElement.getAnnotation(LeanStateProperty::class.java)
            ?.initializer
